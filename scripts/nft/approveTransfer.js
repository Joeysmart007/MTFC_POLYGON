...
async function main() {
  ...
  const JTokenNFT = await hre.ethers.getContractFactory("JTokenNFT");
  const jtTokenNFT = await JTokenNFT.attach(process.env.CONTRACT_ADDRESS);
  ...
  const approveTx = await jtTokenNFT
    .connect(deployer)
  ...
  for (let i = 0; i < 5; i++) {
    const depositTx = await fxRootContract
      .connect(deployer)
      .deposit(jtTokenNFT.address, process.env.BRIDGE_ADDRESS, i, "0x6566");
